    ...
    <div>
      ...
      <div class="relative">
        <input type="number" [value]="value"
               (input)="onInput($event)"
               class="w-full bg-sand/30 rounded-[14px] px-3 py-2 text-sm text-petrol border border-warm-border focus:border-earth focus:outline-none focus:ring-1 focus:ring-earth/30 pr-12" />
        @if (suffix) {
...
@Component({
  ...
  template: `
    <div class="p-4 lg:p-6 space-y-5 bg-cream min-h-full max-w-2xl lg:max-w-5xl mx-auto overflow-y-auto">
      ...
      <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
        @for (tab of tabs; track tab.key) {
          <button (click)="activeTab.set(tab.key)"
                  ...
                  [class]="activeTab() === tab.key ? 'bg-petrol text-white shadow-sm' : 'bg-white text-stone border border-warm-border'">
            {{ tab.label }}
          </button>
        }
      </div>
        ...
        <div class="bg-white rounded-[24px] border border-warm-border shadow-sm p-5 space-y-4">
          ...
          <div class="bg-sand/40 rounded-[14px] p-4 space-y-3">
            ...
            <div class="border-t border-warm-border pt-3">
              ...
              <div class="grid grid-cols-3 lg:grid-cols-4 gap-3 mt-3">
